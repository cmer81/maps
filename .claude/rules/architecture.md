...
`agroclimato_france` (`agroclimato-france-domain.ts`) est le **seul domaine maison à granularité journalière** : 7 indices agroclimatologiques (`temperature_2m_min/max`, `frost_hours`, `heat_hours`, `et0_fao`, `vpd`, `thi_livestock`), un OMfile **par jour** nommé `YYYY-MM-DD.om` (= minuit UTC), même grille que `arome_france` (1121×717 à 0,025°). Trois adaptations seulement, le reste suit le pattern bucket : (1) `time_interval: 'daily'` → la timeline affiche les jours (`isDailyDomain` dans `time-selector.svelte`, désormais dérivé de `time_interval === 'daily'` plutôt que d'une liste de domaines, ce qui couvre aussi l'anomalie) ; (2) `getOMUrlFor()` nomme le fichier via `fmtDateYMD()` au lieu de `fmtSelectedTime()` pour les domaines listés dans `DAILY_FILE_DOMAINS` (`constants.ts`) — le run reste path-style `…/0600Z/` ; (3) palettes dédiées dans `color-scales/agroclimato.ts` (clés exactes dans `standardColorScales`), libellés FR dans `i18n/variables-fr.ts`. **Pas de resolver custom** : le regex timestamp du package (`YY-MM-DDTHH00`) ne matche pas le fichier daté mais le rendu fonctionne quand même (vérifié via l'anomalie, déjà en fichiers `{date}.om`). Quirk producteur : `frost_hours`/`heat_hours` valent **0** (pas NaN) sur l'océan → le 1ᵉʳ breakpoint des palettes est posé à 1 h, donc `< 1 h` est rendu transparent (océan masqué « gratuitement »).

**Piège run-non-aligné (résolu).** Le run agroclimato est à **06:00Z** alors que ses `valid_times` sont des **minuits UTC**. `checkClosestModelRun()` (`time-selector.svelte`) dérivait le run via `closestModelRun(timeStep, 'daily')` (aligné sur minuit) : dès qu'on sélectionnait un `valid_time` antérieur à l'heure du run (le 1ᵉʳ jour 00Z précède le run 06Z), la branche `timeStep < $modelRun` recalait `$modelRun` sur ce minuit → `getMetaData()` fetchait `…/0000Z/meta.json` **inexistant → toast « HTTP 404 »** (le `.om` était bon, c'est le `meta.json` qui 404). Fix : un **early-return** gardé sur `DAILY_FILE_DOMAINS` clampe simplement le temps dans `[metaFirstTime, metaLastTime]` (avec re-sync URL/scrubber) et **reste sur le run latest**, sans passer par `closestModelRun`. L'anomalie n'est pas concernée (run aligné minuit + layout `/anomaly/` propre) et suit le chemin générique. Tout futur domaine maison journalier à run non aligné doit être ajouté à `DAILY_FILE_DOMAINS` pour bénéficier du même traitement.
