...
`src/lib/slot-manager.ts` implements an A/B slot system used by `src/lib/layers.ts`. MapLibre's `setUrl`/`setTiles` does not reliably abort in-flight requests, so each data update creates a new source+layers in the pending slot, waits for `source.loaded()`, then cross-fades opacity and removes the old slot after `removeDelayMs`. When changing rendering code, preserve this pattern — there are now three managers: `rasterManager` (couche raster principale), `rasterManager2` (overlay raster secondaire optionnel, conditionné par `layer2Enabled`) et `vectorManager` (contours/flèches). Tous sont créés dans `createManagers()` et doivent être mis à jour via `update()`/`setBeforeLayer()`, jamais en mutant sources/layers directement.
