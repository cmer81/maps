...
- **Dépendance forkée** (`@openmeteo/weather-map-layer` → `@cm3r/weather-map-layer` ; `file-reader` et `file-format-wasm` sont désormais **officiels**). Si une modif ne peut se faire côté `maps` et exige de toucher la lib : vérifier d'abord qu'elle n'est pas faisable via `omProtocolSettings` (color scales, `resolveRequest`…), sinon modifier le **fork**, garder le patch petit/isolé et rebasable sur l'upstream, puis **publier une nouvelle version npm et bumper le pin ici** — jamais consommer une lib forkée autrement que par une version npm publiée + épinglée (reproductibilité CI). Workflow détaillé : section « Dépendance forkée » du `README.md`.
