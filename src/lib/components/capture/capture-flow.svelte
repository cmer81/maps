...
{#if variant === 'fab'}
	<button
		...
		disabled={busy}
		aria-label={label}
		class="bg-primary flex size-11 cursor-pointer items-center justify-center rounded-full text-white shadow-lg ring-1 ring-white/20 disabled:cursor-not-allowed disabled:opacity-60"
	>
...
{:else}
	<button
		...
		disabled={busy}
		aria-label={label}
		class="bg-primary flex h-8 cursor-pointer items-center gap-1.5 rounded-lg px-3 text-sm font-semibold text-white shadow-md ring-1 ring-white/20 disabled:cursor-not-allowed disabled:opacity-60"
	>
